import pygame
import random
import sys

pygame.init()

# Tamaño del tablero
GRID_SIZE = 8
CELL_SIZE = 50

LEFT_MARGIN = 60
TOP_MARGIN = 80
BOARD_SPACING = 120

WIDTH = GRID_SIZE * CELL_SIZE * 2 + BOARD_SPACING + LEFT_MARGIN * 2
HEIGHT = GRID_SIZE * CELL_SIZE + TOP_MARGIN * 2

screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Batalla Naval - Versión 0 (Spaghetti sin funciones)")

# Colores
BLUE = (80, 140, 220)
WHITE = (240, 240, 240)
BLACK = (0, 0, 0)
RED = (220, 60, 60)
GRAY = (180, 180, 180)
BACKGROUND = (220, 220, 240)

# Estados
EMPTY = 0
SHIP = 1
MISS = 2
HIT = 3

# Flota simple
FLEET = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]

# Tableros
player_board = [[0]*GRID_SIZE for _ in range(GRID_SIZE)]
enemy_board = [[0]*GRID_SIZE for _ in range(GRID_SIZE)]
enemy_view = [[0]*GRID_SIZE for _ in range(GRID_SIZE)]

# Listas de barcos (cada barco es lista de (r, c))
player_ships = []   # <<< NUEVO
enemy_ships = []    # <<< NUEVO

font = pygame.font.SysFont(None, 28)
big_font = pygame.font.SysFont(None, 36)

# ---------------------------
# COLOCACIÓN DE FLOTAS
# ---------------------------

for board in (player_board, enemy_board):
    for size in FLEET:
        placed = False
        while not placed:
            r = random.randint(0, GRID_SIZE - 1)
            c = random.randint(0, GRID_SIZE - 1)
            horizontal = random.choice([True, False])

            ok = True
            if horizontal:
                if c + size > GRID_SIZE:
                    ok = False
                else:
                    for i in range(size):
                        if board[r][c+i] != EMPTY:
                            ok = False
            else:
                if r + size > GRID_SIZE:
                    ok = False
                else:
                    for i in range(size):
                        if board[r+i][c] != EMPTY:
                            ok = False

            if ok:
                cells = []
                if horizontal:
                    for i in range(size):
                        board[r][c+i] = SHIP
                        cells.append((r, c+i))
                else:
                    for i in range(size):
                        board[r+i][c] = SHIP
                        cells.append((r+i, c))

                # Guardar barco en la lista correspondiente
                if board is player_board:
                    player_ships.append(cells)
                else:
                    enemy_ships.append(cells)

                placed = True

# ---------------------------
# FUNCIÓN: obtener rectángulos de barcos
# ---------------------------

def get_ship_rectangles(ships, offset_x, offset_y):
    rects = []
    for ship in ships:
        rows = [r for r, _ in ship]
        cols = [c for _, c in ship]
        min_r, max_r = min(rows), max(rows)
        min_c, max_c = min(cols), max(cols)

        x = offset_x + min_c * CELL_SIZE
        y = offset_y + min_r * CELL_SIZE
        w = (max_c - min_c + 1) * CELL_SIZE
        h = (max_r - min_r + 1) * CELL_SIZE

        rects.append((ship, pygame.Rect(x, y, w, h)))
    return rects

# ---------------------------
# VARIABLES DE JUEGO
# ---------------------------

player_turn = True
game_over = False
winner = None

clock = pygame.time.Clock()

# ---------------------------
# BUCLE PRINCIPAL
# ---------------------------

while True:
    clock.tick(30)

    # Eventos
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()

        # Disparo del jugador
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and player_turn and not game_over:
            mx, my = event.pos
            enemy_x = LEFT_MARGIN + GRID_SIZE * CELL_SIZE + BOARD_SPACING
            enemy_y = TOP_MARGIN

            if mx > enemy_x and my > enemy_y:
                c = (mx - enemy_x) // CELL_SIZE
                r = (my - enemy_y) // CELL_SIZE

                if 0 <= r < GRID_SIZE and 0 <= c < GRID_SIZE:
                    if enemy_view[r][c] == EMPTY:
                        if enemy_board[r][c] == SHIP:
                            enemy_board[r][c] = HIT
                            enemy_view[r][c] = HIT

                            # Comprobar hundimiento total
                            sunk = True
                            for rr in range(GRID_SIZE):
                                for cc in range(GRID_SIZE):
                                    if enemy_board[rr][cc] == SHIP:
                                        sunk = False
                            if sunk:
                                game_over = True
                                winner = "Jugador"

                        else:
                            enemy_view[r][c] = MISS
                            player_turn = False

    # Turno de la IA
    if not player_turn and not game_over:
        pygame.time.delay(400)

        while True:
            r = random.randint(0, GRID_SIZE - 1)
            c = random.randint(0, GRID_SIZE - 1)
            if player_board[r][c] in (EMPTY, SHIP):
                break

        if player_board[r][c] == SHIP:
            player_board[r][c] = HIT

            sunk = True
            for rr in range(GRID_SIZE):
                for cc in range(GRID_SIZE):
                    if player_board[rr][cc] == SHIP:
                        sunk = False
            if sunk:
                game_over = True
                winner = "IA"

        else:
            player_board[r][c] = MISS
            player_turn = True

    # ---------------------------
    # DIBUJO
    # ---------------------------

    screen.fill(BACKGROUND)

    player_x = LEFT_MARGIN
    player_y = TOP_MARGIN
    enemy_x = LEFT_MARGIN + GRID_SIZE * CELL_SIZE + BOARD_SPACING
    enemy_y = TOP_MARGIN

    screen.blit(big_font.render("Tu tablero", True, BLACK), (player_x, player_y - 40))
    screen.blit(big_font.render("Tablero enemigo", True, BLACK), (enemy_x, enemy_y - 40))

    # --- TABLERO DEL JUGADOR ---
    pygame.draw.rect(screen, BLACK, (player_x-5, player_y-5, GRID_SIZE*CELL_SIZE+10, GRID_SIZE*CELL_SIZE+10), 3)
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            cell = player_board[r][c]
            color = BLUE
            if cell == MISS:
                color = WHITE
            elif cell == HIT:
                color = RED
            elif cell == SHIP:
                color = GRAY

            pygame.draw.rect(screen, color, (player_x + c*CELL_SIZE, player_y + r*CELL_SIZE, CELL_SIZE, CELL_SIZE))
            pygame.draw.rect(screen, BLACK, (player_x + c*CELL_SIZE, player_y + r*CELL_SIZE, CELL_SIZE, CELL_SIZE), 1)

    # Marcos de barcos del jugador (siempre)
    for cells, rect in get_ship_rectangles(player_ships, player_x, player_y):
        pygame.draw.rect(screen, WHITE, rect, 4)

    # --- TABLERO ENEMIGO ---
    pygame.draw.rect(screen, BLACK, (enemy_x-5, enemy_y-5, GRID_SIZE*CELL_SIZE+10, GRID_SIZE*CELL_SIZE+10), 3)
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            cell = enemy_view[r][c]
            color = BLUE
            if cell == MISS:
                color = WHITE
            elif cell == HIT:
                color = RED

            pygame.draw.rect(screen, color, (enemy_x + c*CELL_SIZE, enemy_y + r*CELL_SIZE, CELL_SIZE, CELL_SIZE))
            pygame.draw.rect(screen, BLACK, (enemy_x + c*CELL_SIZE, enemy_y + r*CELL_SIZE, CELL_SIZE, CELL_SIZE), 1)

    # Marcos en tablero enemigo solo si el barco está hundido
    for cells, rect in get_ship_rectangles(enemy_ships, enemy_x, enemy_y):
        if all(enemy_view[r][c] == HIT for r, c in cells):
            pygame.draw.rect(screen, WHITE, rect, 4)

    if game_over:
        msg = big_font.render(f"Fin del juego: {winner} gana", True, BLACK)
        screen.blit(msg, (WIDTH//2 - msg.get_width()//2, HEIGHT - 60))

    pygame.display.flip()
